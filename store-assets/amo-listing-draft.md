...
## Reviewer Notes
This extension runs only on `https://www.youtube.com/*`, and caption work is additionally gated to YouTube watch pages with a valid video id. It reads YouTube captions/transcript/timed-text state locally to render a dialogue-style overlay. It does not collect, transmit, sell, export, or store caption text, watched videos, browsing history, account data, or analytics events. It stores only local UI preferences using extension storage: color, opacity, fade, and optional locked layout preferences. No server, account, analytics, ads, tracking, payment, or remote code services are used.
