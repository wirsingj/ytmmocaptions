...
exports.run = async function runComplianceTests(ctx) {
  ...
  await runCase("release automation is tag-gated and manually approved before store publishing", () => {
    ...
    assert.ok(release.includes("chrome_action:"));
    assert.ok(release.includes("firefox_action:"));
    assert.ok(release.includes("environment: store-publish"));
    assert.ok(release.includes("github.event_name == 'workflow_dispatch'"));
    assert.ok(release.includes("inputs.release_tag"));
    assert.ok(release.includes("scripts/verify-release-version.mjs"));
    assert.ok(release.includes("CHROME_STORE_ACTION"));
    assert.ok(release.includes("FIREFOX_STORE_ACTION"));
    ...
    assert.ok(releaseDocs.includes("chrome_action: skip | upload | publish"));
    assert.ok(releaseDocs.includes("firefox_action: skip | publish"));
