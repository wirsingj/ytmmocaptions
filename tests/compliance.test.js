...
exports.run = async function runComplianceTests(ctx) {
  ...
  await runCase("release automation is tag-gated and manually approved before store publishing", () => {
    ...
    const publishChrome = fs.readFileSync(path.join(ROOT_DIR, ".github", "workflows", "publish-chrome.yml"), "utf8");
    const publishFirefox = fs.readFileSync(path.join(ROOT_DIR, ".github", "workflows", "publish-firefox.yml"), "utf8");
    ...
    assert.ok(release.includes("git merge-base --is-ancestor"));
    assert.ok(!release.includes("workflow_dispatch:"));
    assert.ok(!release.includes("environment: store-publish"));
    assert.ok(!release.includes("CHROME_CLIENT_SECRET"));
    assert.ok(publishChrome.includes("name: Publish Chrome Web Store"));
    assert.ok(publishChrome.includes("workflow_dispatch:"));
    assert.ok(publishChrome.includes("chrome_action:"));
    assert.ok(publishChrome.includes("environment: store-publish"));
    assert.ok(publishChrome.includes("scripts/verify-release-version.mjs"));
    assert.ok(publishChrome.includes("git merge-base --is-ancestor"));
    assert.ok(publishChrome.includes("CHROME_STORE_ACTION"));
    assert.ok(publishChrome.includes("CHROME_PUBLISHER_ID"));
    assert.ok(publishChrome.includes("CHROME_EXTENSION_ID"));
    assert.ok(publishChrome.includes("CHROME_CLIENT_SECRET"));
    assert.ok(publishChrome.includes("is configured."));
    assert.ok(publishChrome.includes("Missing Chrome Web Store secrets"));
    assert.ok(publishFirefox.includes("name: Publish Firefox AMO"));
    assert.ok(publishFirefox.includes("workflow_dispatch:"));
    assert.ok(publishFirefox.includes("environment: store-publish"));
    assert.ok(publishFirefox.includes("scripts/verify-release-version.mjs"));
    assert.ok(publishFirefox.includes("git merge-base --is-ancestor"));
    assert.ok(publishFirefox.includes("AMO_JWT_ISSUER"));
    assert.ok(publishFirefox.includes("AMO_JWT_SECRET"));
    assert.ok(publishFirefox.includes("is configured."));
    assert.ok(publishFirefox.includes("Missing Firefox AMO secrets"));
    assert.ok(releaseDocs.includes("Publish Chrome Web Store"));
    assert.ok(releaseDocs.includes("chrome_action: upload | publish"));
    assert.ok(releaseDocs.includes("Publish Firefox AMO"));
