...
exports.run = async function runComplianceTests(ctx) {
  ...
  await runCase("timeline scrub mode reuses panel chunks and stays optional", () => {
    ...
    assert.ok(panelSource.includes('setAlpha("--dc-panel-alpha-outer", 0.16 + eased * 0.54)'));
    assert.ok(panelSource.includes('setAlpha("--dc-card-alpha", 0.2 + eased * 0.5)'));
    ...
    assert.ok(css.includes("width: min(620px, 72vw)"));
